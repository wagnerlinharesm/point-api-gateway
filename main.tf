...
resource "aws_api_gateway_integration" "report_generate_integration" {
  ...
  request_templates = {
    "application/json" = "Action=SendMessage&MessageBody={ \"authorization\": \"$util.base64Encode($input.params().header.get('Authorization'))\", \"body\": \"$util.base64Encode($input.body)\" }&MessageGroupId=$util.base64Encode($input.params().header.get('Authorization'))&MessageDeduplicationId=$util.base64Encode({ \"authorization\": \"$input.params().header.get('Authorization')\", \"body\": \"$input.body\" })"
  }
}
...
data "aws_lambda_function" "point_save" {
  function_name = "point_save"
}

data "aws_lambda_function" "point_query" {
  function_name = "point_query"
}

resource "aws_api_gateway_resource" "point_resource" {
  rest_api_id = aws_api_gateway_rest_api.point_api_gateway.id
  parent_id   = aws_api_gateway_rest_api.point_api_gateway.root_resource_id
  path_part   = "point"
}

resource "aws_api_gateway_method" "save_method" {
  rest_api_id   = aws_api_gateway_rest_api.point_api_gateway.id
...
resource "aws_api_gateway_integration" "point_save_lambda_integration" {
  rest_api_id             = aws_api_gateway_rest_api.point_api_gateway.id
  resource_id             = aws_api_gateway_resource.point_resource.id
  http_method             = aws_api_gateway_method.save_method.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:us-east-2:lambda:path/2015-03-31/functions/arn:aws:lambda:us-east-2:644237782704:function:point_save/invocations"
}
...
resource "aws_lambda_permission" "point_save_permission" {
  statement_id  = "AllowAPIInvoke"
  action        = "lambda:InvokeFunction"
  function_name = data.aws_lambda_function.point_save.function_name
  principal     = "apigateway.amazonaws.com"
