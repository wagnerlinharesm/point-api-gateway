...
resource "aws_api_gateway_integration" "report_generate_integration" {
  ...
  request_templates = {
    "application/json" = "Action=SendMessage&MessageGroupId=$util.base64Encode($input.params().header.get('Authorization'))&MessageBody={ \"authorization\": \"$util.base64Encode($input.params().header.get('Authorization'))\", \"body\": \"$util.base64Encode($input.body)\" }"
  }
}
